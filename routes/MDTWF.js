...
router.post('/add', authenticationToken, async(req,res)=>{
    try {
        ...
        if(!accountid || !operation || !amount || !transactionDate || !userid ){
            ...
            await pool.query(
                `INSERT INTO mdtwf (accountid, operation, amount, transactionDate, remark, userid)
            ...
            smsHelper(amount, 'MDTWF',operation,member.rows[0].name,member.rows[0].mobile);
            res.status(201).json({status:'success', message:'MDTWF added successfully!'});
        }
    } catch (error) {
...
router.get('/getByOpertion/:operation', authenticationToken, async(req,res)=>{
    try {
        ...
        const mdtwf = await pool.query(`SELECT * FROM mdtwf WHERE operation=$1`,[operation]);
        res.status(200).json({status:'success', mdtwf: mdtwf.rows})
    } catch (error) {
        res.status(400).json({status:'fail',message: error.message});
    }
})
...
router.get('/getById/:accountid', authenticationToken, async(req,res)=>{
    try {
        ...
        const mdtwf = await pool.query(`SELECT * FROM mdtwf WHERE id=$1`,[accountid]);
        res.status(200).json({status:'success', mdtwf: mdtwf.rows})
    } catch (error) {
        res.status(400).json({status:'fail',message: error.message});
    }
...
router.get('/getByUserId/:userid', authenticationToken, async(req,res)=>{
    try {
        ...
        const mdtwf = await pool.query(`SELECT * FROM mdtwf WHERE id=$1`,[userid]);
        res.status(200).json({status:'success', mdtwf: mdtwf.rows})
    } catch (error) {
        res.status(400).json({status:'fail',message: error.message});
    }
...
router.put('/update',authenticationToken, async(req,res)=>{
    try {
        ...
        res.status(200).json({status:'success', message: 'MDTWF updated successfully!'})
    } catch (error) {
        res.status(400).json({status:'fail',message: error.message});
    }
})
